...
function draw3D(I, cads, O, C, alpha, center, cad_label, flag, y, index_show)

if nargin < 10
    index_show = 1:size(O,2);
% else
%     index_show = flag(index_show);
...
for i = 1:num
    ...
    if isfield(y(index), 'class') == 1 && strcmp(y(index).class, 'table') == 1
...
for i = 1:num  
    ...
    part_num = numel(pnames);
    % change the mask
    if isfield(y(flag == ind), 'class') == 0 || strcmp(y(flag == ind).class, 'car') == 1
        parts = zeros(4*part_num, 2);
        for j = 1:part_num
            % only for aspect part
            if cad.roots(j) == 0 && isempty(cad.parts2d(view_index).(pnames{j})) == 0
                % compute part center
                c = y(flag == ind).part_label(j,:) + [par.padx par.pady];
                % part shape
