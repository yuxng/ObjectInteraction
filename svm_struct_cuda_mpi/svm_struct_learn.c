			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  printf("Create a cache of the feature vectors for the correct labels done.\n");
  ...
  { 
    ...
    if(alg_type == DUAL_CACHE_ALG) 
    {
      ...
      update_constraint_cache_for_model(ccache, svmModel, start, end);
      dist = find_most_violated_joint_constraint_in_cache(ccache, &lhs, &margin, start, end, kparm, sm);
    }
...
void update_constraint_cache_for_model(CCACHE *ccache, MODEL *svmModel, int start, int end)
...
double find_most_violated_joint_constraint_in_cache(CCACHE *ccache, SVECTOR **lhs, double *margin, int start, int end, KERNEL_PARM *kparm, STRUCTMODEL *sm)
